...
ADD CONSTRAINT fkpatient2
FOREIGN KEY (patient_id) REFERENCES patients (patient_id);

ALTER TABLE prescriptions 
ADD CONSTRAINT fkdoctor2
FOREIGN KEY (doctor_id) REFERENCES doctors (doctor_id);

ALTER TABLE prescriptions 
ADD CONSTRAINT fkmedication2
...
-- Part 2: SQL Queries to Analyze Hospital Data
...
SELECT * 
FROM medications
WHERE description like "%infection%" OR description LIKE "%pain%";
...
WHERE MONTH(a.appointment_date) = 8 AND 
	  YEAR(a.appointment_date) = 2025
ORDER BY p.name;
...
SELECT m.name AS medication_name, prescription_count
FROM (
    SELECT 
        medication_id,
...
-- Any further insights derived from the data:

-- Top hospitals with the most specialists in a particular field (e.g., cardiology)
...
SELECT h.name, AVG(r.capacity) AS avg_capacity
FROM hospitals h
JOIN departments d ON h.hospital_id = d.hospital_id
JOIN rooms r ON d.department_id = r.department_id
GROUP BY h.name;
